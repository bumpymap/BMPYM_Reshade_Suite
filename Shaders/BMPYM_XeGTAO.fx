...
//                                                            //
// XeGTAO - Approximate Ground Truth Ambient Occlusion        //
// IT is an approximation of legitimate ambient occlusion     //
//                                                            //
// Intel's Open Source https://github.com/GameTechDev/XeGTAO  //
//                                                            //
// My implementation is inspired by Acerola FX                //
...
#include "Include/BMPYM_XeGTAO.fxh"
...
uniform float u_Effect_Falloff_Range <
    ui_category        = "SSAO Settings";
    ui_category_closed = true;
    ui_min = 0.01f; ui_max = 5.0f;
...
uniform float u_Sample_Distribution_Power <
    ui_category        = "SSAO Settings";
    ui_category_closed = true;
...
uniform float u_Thin_Occluder_Compensation <
    ui_category        = "SSAO Settings";
    ui_category_closed = true;
...
uniform float u_Slope_Compensation <
    ui_category        = "SSAO Settings";
    ui_category_closed = true;
    ui_min = 0.0f; ui_max = 1.0f;
    ui_label   = "Slope Compensation";
    ui_tooltip = "Compensate for dark slopes";
    ui_type    = "drag";
> = 0.05f;

uniform float u_Final_Value_Power <
    ui_category        = "SSAO Settings";
    ui_category_closed = true;
    ui_min = 0.01f; ui_max = 5.0f;
...
uniform float u_SigmaD <
    ...
    ui_label   = "SigmaD";
    ui_type    = "drag";
    ui_tooltip = "Modify the distance of bilateral filter samples";
> = 1.0f;
...
//float4 PS_EndPass(float4 position : SV_POSITION, float2 uv : TEXCOORD) : SV_TARGET { return tex2D(Common::RenderTex, uv); }
...
//**************************************************//
//                  FUNCTIONS                       //
//  Consists of calculation functions, compute      //
//  shader functions and pixel shader pass          //
//**************************************************//
...
void CS_MainPass(uint3 tid : SV_DISPATCHTHREADID) 
{
    ...
    const float effectRadius = u_Effect_Radius * u_Radius_Multiplier;
    const float sampleDistributionPower = u_Sample_Distribution_Power;
    const float thinOccluderCompensation = u_Thin_Occluder_Compensation;
    const float falloffRange = u_Effect_Falloff_Range * effectRadius;

    const float falloffFrom = effectRadius * (1.0f - u_Effect_Falloff_Range);
    ...
    for (float slice = 0; slice < sliceCount; ++slice) {
        ...
        projectedNormalVecLength = lerp(projectedNormalVecLength, 1.0f, u_Slope_Compensation);
    ...
    visibility = pow(abs(visibility), u_Final_Value_Power);
...
{
    const int kernelRadius = (int)ceil(2.0f * u_SigmaD);
    ...
    for (int x = lower; x <= upper; ++x) {
        for(int y = lower; y <= upper; ++y) {
            ...
            float weight = gaussD(u_SigmaD, (tid.x + x) - tid.x, (tid.y + y) - tid.y) * gaussR(u_SigmaR, intKerPos - center);
    ...
    // Apply AO
    float4 col = tex2Dfetch(ReShade::BackBuffer, tid.xy);
    ...
    #if DEBUG_SSAO == 1
        tex2Dstore(Common::s_RenderTex, tid.xy, visibility);
    #else
        tex2Dstore(Common::s_RenderTex, tid.xy, float4(col.rgb * output, col.a));
    #endif
}
